        ...
        public void run() {
            for (Player p : Bukkit.getOnlinePlayers()) {
                if (main.getTimerManager().hasPlayerTimer(p)) {
                    ...
                    if ((WalkableBlocksUtil.noContainsWalkable(session.getParkour().getWalkableBlocks(), block.getType().name(), block.getData()) && block.getType() != Material.HEAVY_WEIGHTED_PRESSURE_PLATE && block.getType() != Material.LIGHT_WEIGHTED_PRESSURE_PLATE && block.getType() != Material.AIR)) {
